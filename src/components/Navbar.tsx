...
const Navbar: FC = () => {
  ...
  // Statsig dark mode feature gate
  const darkModeEnabled = useGateValue("dark_mode");
  useEffect(() => {
    if (darkModeEnabled) {
      ...
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-4 md:gap-8 items-center dark:text-white">
            ...
            {/* Dark mode icon controlled by Statsig feature gate */}
            {darkModeEnabled ? (
              <MdOutlineDarkMode className="text-blue-500" size={30} />
            ) : (
              <MdOutlineLightMode className="text-gray-400" size={30} />
            )}
          </div>
        </div>
      </div>
